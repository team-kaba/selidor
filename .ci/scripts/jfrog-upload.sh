...
jfrog rt u --spec .ci/jfrog-cli-rt-upload-spec.json --spec-vars "dist-dir=${BUILT_ARTIFACTS_DIR};type=${type}" --module "${module_name}" --props "artifactory.licenses=${BINTRAY_DEFAULT_LICENSES}"
...
echo "Publishing build information of atrtifacts to ${ARTIFACTORY_SERVER_ID} using jfrog cli."
jfrog rt u --spec .ci/jfrog-cli-rt-upload-spec.json --spec-vars "dist-dir=${BUILT_ARTIFACTS_DIR};type=${type}" --module "${module_name}" --props "artifactory.licenses=${BINTRAY_DEFAULT_LICENSES}" --dry-run
jfrog rt bce
jfrog rt bag --config "${PROJECT_ROOT_DIR}/.ci/jfrog-cli-bag-config-${type}.yml" "${PROJECT_ROOT_DIR}" || echo "Failed to collect git information."
jfrog rt bp --dry-run
